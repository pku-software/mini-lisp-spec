...
本文档所述 *错误情形*，是指当本文档使用 *发出错误信号* 这一术语的情形。此时，实现必须报告错误。错误信号被发出时，实现应当表现为 `error` 过程被执行。当数据被 `error` 过程引用时，实现应当报告该数据。报告格式是实现定义的；建议以该数据的外部表示作为报告格式的一部分。
...
分号字符（`;`，Unicode 码位 U+003B）视为 *注释* 的起始字符。注释的终止字符为下一组操作系统定义的换行符序列的最后一个字符。注释之中的任何字符在程序 *执行* 期间都应当被忽略。允许实现定义任何其它形式的注释语法，但禁止与本文档所述的语法规则冲突。
...
解释流程在某一时刻可用的所有绑定构成的集合，称为该时刻的 *环境*。
...
所有数据都存在一种 *外部表示*。对某一数据的外部表示 *求值* 总是得到该数据本身。
...
实现可以假设所有的存储空间只会在绑定时被写入（即只读的）。允许实现定义对存储空间的写操作。写操作应当以内置过程的形式提供接口，且内置过程的默认标识符名称应当以字符 `!`（Unicode 码位 U+0021）结尾。

## 表达式

Mini-Lisp 程序由若干 *表达式* 构成。*表达式* 是由一系列语法记号构成的，可以被 *求值* 的程序文本。表达式包括 *初等表达式* 和 *派生表达式* 两个类别。实现可以提供更多的表达式形式。

> 表达式通常具有 *列表* 的外部表示形式或字面量形式。
...
过程调用写作一对小括号。首先，对小括号内的首个表达式求值，得到被 *调用* 的过程。随后，其余表达式从前至后依次求值后，做为传入该过程的实参。
...
过程调用求值的结果，是被 *调用* 的过程所定义的 *返回值*。该过程可以为本文档所定义的库的过程，则返回值由本文档所定义。

对空的过程调用形式（即 `(` `)`）求值是未指明行为：要么返回空表，要么发出错误信号。

当过程的操作数为变量名（标识符）时，可能与本文档所定义的其它语法相冲突。这些语法具有过程的形式，称为 *特殊形式*。
...
该特殊形式求值得到一个过程类型的数据——*过程*。过程可以被 *调用*。当过程被调用时，将程序当前的环境替换为过程定义时所在的环境，并添加 *参数绑定*。

*参数绑定* 为过程调用的环境添加若干绑定。对于一般形参形式中的每一个标识符，将其绑定到调用时传入的实参值上。在一般形参形式中，如果形参标识符的数量与实参的数量不符，则发出错误信号。

随后，对 $\lang$ 过程体 $\rang$ 的每个表达式进行求值。将最后一个表达式的求值结果作为过程的 *返回值*。最后，恢复到过程调用时的环境。
...
若 *val* 是字符串类型数据，则将字符串内容通过 *标准输出* 输出；否则如同执行 `(` `print` *val* `)`。返回值：未定义；建议空表。
...
向 *标准输出* 输出 *val* 的外部表示。实现可以在外部表示前添加单引号 `'`。返回值：未定义；建议空表。
