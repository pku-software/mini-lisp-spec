...
> **版本 20240619** [谷雨同学](https://guyutongxue.site)制订
...
- 可以提供不在本文档描述的库之外的标识符；实现 *必须* 为此类标识符提供文档说明；
- 对于本文档所定义的标识符，可以提供额外的功能，但禁止与本文档所述的内容冲突；实现 *必须* 为此类功能提供文档说明。
...
`and` 特殊形式被求值时，从前至后一次求值每一个 $\lang$ 表达式 $\rang$，直至该表达式求值结果为 *虚值*，并将该结果作为整个形式的求值结果。如果没有表达式求值得到 *虚值*，那么返回最后一个表达式的求值结果。如果特殊形式内不存在任何表达式，求值为 `#t`。

`or` 特殊形式被求值时，从前至后一次求值每一个 $\lang$ 表达式 $\rang$，直至该表达式求值结果不是 *虚值*，并将该结果作为整个形式的求值结果。如果所有表达式求值都得到 *虚值*，那么返回最后一个表达式的求值结果。如果特殊形式内不存在任何表达式，求值为 `#f`。
...
每次 *标准输入* 收到数据后，实现应当解析输入直到内容构成某一数据的外部表示。如果部分输入数据已经可以判断为不符合任何类型数据的外部表示，则可以积极发出错误信号。若单次输入的全部输入数据未构成完整某一数据的外部表示，或解析后尚有冗余的输入数据，实现的行为是未定义的。
...
数类型的数据之间是否具有相等性，等价于它们在实现定义的精度下所表示的数值是否相同。

> 注：如果实现采用 IEEE 754 双精度浮点类型存储数类型数据，那么 `+0` 和 `-0` 是否具有相等性是未指明的；`NaN` 和其它数的相等性是未指明的。
...
以下过程定义中，若实参不符合形参名于 [§1.3](#凡例) 中所规定的期望实参类型或实现定义的期望实参类型，实现应当发出错误信号。

> 注：若实现支持以下过程定义传入其它类型实参或其它长度的实参个数，则必须提供文档说明。
...
以 *n* 作为进程退出码退出解释流程，*n* 的默认值为 `0`。返回值：绝不返回。
