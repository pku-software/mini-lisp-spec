...
> **版本 20230306** [谷雨同学](https://guyutongxue.site)制订
...
参数绑定创建一个新环境，其上级环境定义为该特殊形式定义所在的环境。随后，为次新环境添加变量绑定：对于一般形参形式中的每一个标识符，将其绑定到调用时传入的实参值上。在一般形参形式中，如果形参标识符的数量与实参的数量不符，则发出错误信号。
